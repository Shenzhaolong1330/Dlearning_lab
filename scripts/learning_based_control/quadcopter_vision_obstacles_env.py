...
.. code-block:: bash
    ...
    # Usage
    python scripts/learning_based_control/quadcopter_vision_obstacles_env.py --enable_camera
...
def generate_random_position(index: int, region: dict = {'x':10, 'y':10, 'z':10}, min_spacing=1.0):
    positions = []
        ...
        rigid_objects={
            f"obstacle_{i}": RigidObjectCfg(
                ...
                spawn=sim_utils.CuboidCfg(
                    size=(random.uniform(0.3, 0.5), random.uniform(0.3, 0.5), random.uniform(0.7, 1.5)), 
...
def main():
    ...
    depth_image_dir = "scripts/learning_based_control/depth_images"
    ...
    while count < 300:
        ...
        #     env_depth_smoothed = gaussian_blur(env_depth, kernel_size=[5, 5], sigma=[1000.0, 1000.0])
